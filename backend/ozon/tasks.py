...
def update_abc_sheet(spreadsheet_url: str = None, sa_json_path: str = None):
    """
        ...
        for campaign in auto_campaigns:
            if campaign.sku and campaign.sku not in auto_campaigns_dict:
                ...
                auto_campaigns_dict[campaign.sku] = {
                    'name': campaign.name or campaign.offer_id,
    ...
    try:
        ...
        for r in selected:
            ...
            # Формируем название кампании с учетом статуса для существующих кампаний
        ...
        for campaign in manual_campaigns:
            ...
            for sku in all_skus_in_campaign:
                ...
                campaign_names.append([campaign_name_with_date])
...
def fetch_campaigns_from_ozon(store: OzonStore) -> list:
    ...
    Returns:
        Список кампаний или пустой список при ошибке
    """
    try:
...
def read_google_sheets_data(spreadsheet_url: str = None, sa_json_path: str = None, worksheet_name: str = "Main_ADV", start_row: int = 13, block_size: int = 100):
    """
        ...
        for ad_data in data_rows:
            ...
            if not campaign_id:
                ...
                try:
                    ...
                    logger.info(f"[🚀] Создаем кампанию для SKU {sku}: '{campaign_name}', бюджет: {week_budget_float}")
                ...
                try:
                    # 1. Проверяем в ручных кампаниях
                    manual_campaign = ManualCampaign.objects.filter(
                        store=store, 
                        ozon_campaign_id=campaign_id
                    ).first()
                    ...
                    if manual_campaign:
                        logger.debug(f"[⏭️] Строка {ad_data['row_number']}: найдена ручная кампания (ID: {campaign_id}), пропускаем")
                        campaigns_skipped += 1
                        continue
                    
                    # 2. Проверяем в автоматических кампаниях
                    ...
                    if auto_campaign:
                        ...
                        if campaign_age_days >= train_days:
                            # Кампания старше времени обучения - обновляем бюджет
                            try:
                                week_budget = ad_data['week_budget']
                            ...
                            # Кампания еще в периоде обучения
                            logger.debug(f"[📚] Кампания {campaign_id} еще в периоде обучения ({campaign_age_days}/{train_days} дней), пропускаем")
                            campaigns_skipped += 1
                    else:
...
def sync_campaign_activity_with_sheets(spreadsheet_url: str = None, sa_json_path: str = None, worksheet_name: str = "Main_ADV", start_row: int = 13, block_size: int = 100):
    """
    ...
    try:
        ...
        while empty_rows_count < max_empty_rows:
            ...
            # Читаем блок данных: A (campaign_id), B (active), C (sku), I (manual_budget)
            try:
                ...
                for i, row in enumerate(values):
                    ...
                    sku_value = str(row[2]).strip() if len(row) > 2 else ""
                    ...
                    logger.debug(f"[🔍] Строка {row_number}: campaign_id={campaign_id}, active={active_value}, sku={sku_value}, manual_budget={manual_budget_value}")
                    ...
                    # Проверяем изменения в ручном бюджете (колонка I)
                    budget_updated = False
                    if manual_budget_value:
                        try:
                            manual_budget_float = float(manual_budget_value)
                            current_manual_budget = float(ad_plan_item.manual_budget)
                            
                            # Если бюджеты отличаются, обновляем
                            if abs(manual_budget_float - current_manual_budget) > 0.01:  # Учитываем погрешность float
                                logger.info(f"[💰] Кампания {campaign_id}: обновление ручного бюджета {current_manual_budget} -> {manual_budget_float}")
                                ...
                                try:
                                    ...
                                    ad_plan_item.day_budget = manual_budget_float / 7
