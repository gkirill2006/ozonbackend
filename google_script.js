function updateOzonAnalytics() {
  ...
  for (const cluster of json.clusters) {
    ...
    for (const product of products) {
      ...
      sheet.getRange(row, 25).setValue(product.for_delivery);   
      sheet.getRange(row, 25).setBackground("#FFFF00");
  ...
  let row_summary = 16;
   for (const summary of json.summary) {
    ...
    if (summary.total_for_delivery > 0){
      sheet.getRange(row_summary, 26).setBackground("#D9EAD3")
      sheet.getRange(row_summary, 27).setBackground("#D9EAD3")

    }
    else{
      sheet.getRange(row_summary, 26).setBackground("#F4CCCC")
      sheet.getRange(row_summary, 27).setBackground("#F4CCCC")

    }
    sheet.getRange(row_summary, 26).setValue(summary.offer_id);       
    sheet.getRange(row_summary, 27).setValue(summary.total_for_delivery); 
    row_summary++      
   }
...
/**
 * Получает артикулы товаров для исключения из столбца AE
...
function getExcludeOfferIds(sheet) {
  ...
  // Читаем данные из столбца AE начиная с ячейки AE16
  while (true) {
    const cellValue = sheet.getRange(`AE${row}`).getValue();
  ...
  console.log(`Найдено артикулов для исключения: ${uniqueOfferIds.length}`);
  if (uniqueOfferIds.length > 0) {
    console.log(`Артикулы для исключения: ${uniqueOfferIds.join(', ')}`);
  }
...
/**
 * Получает обязательные товары из столбцов AF-AG
 * @param {Sheet} sheet - Активный лист
...
function getMandatoryProducts(sheet) {
  ...
  // Читаем данные из столбцов AF (артикул) и AG (количество) начиная с ячейки AF16
  while (true) {
    const offerId = sheet.getRange(`AF${row}`).getValue();
    const quantity = sheet.getRange(`AG${row}`).getValue();
  ...
  console.log(`Найдено обязательных товаров: ${mandatoryProducts.length}`);
  if (mandatoryProducts.length > 0) {
    console.log(`Обязательные товары:`, mandatoryProducts);
  }
